...
const ARCADE_MAP = {
  AXIS_X: 0, // Links/Rechts
  AXIS_Y: 1, // Hoch/Runter
  BTN_A: 1, // B1 (Unten?)
  BTN_B: 2, // B2 (Rechts?)
  BTN_X: 0, // B0 (Links?)
  BTN_Y: 3, // B3 (Oben?)
  BTN_L: 4, // Schultertaste L
  BTN_R: 5, // Schultertaste R
  BTN_SELECT: 8, // Select
  BTN_START: 9, // Start
};
...
export class GameComponent implements AfterViewInit, OnDestroy {
  ...
  private readonly CHART_BG_WIDTH = 790; // Breite des Background-Patterns
  private chartBgImage: HTMLImageElement | null = null;

  // === X-AXIS SCROLLING ===
  public xAxisOffset = 0;
  private readonly X_AXIS_SCROLL_STEP = 2.0;
  private readonly X_AXIS_REPEAT_WIDTH = 7000;
  ...
  private preloadImages() {
    ...
    const assets = [
      ...
      { img: this.chartBgImage, src: '/assets/images/Chart.PNG' },
    ];
    let loadedCount = 0;
    assets.forEach((asset) => {
      asset.img.onload = () => {
        loadedCount++;
        this.checkLoadStatus(loadedCount, assets.length);
      };
      asset.img.onerror = () => {
        loadedCount++;
        this.checkLoadStatus(loadedCount, assets.length);
  ...
  get showGameScene(): boolean {
    return this.gameState === 'running' || this.gameState === 'paused';
  }
  trackByFn(index: number, item: SpecialItem): number {
    return item.id;
  }
  ...
  get isTwoPlayer(): boolean {
    return this.gameMode === 'twoPlayer';
  }
  ...
  private startGameLoop = (): void => {
    ...
    // LEVEL
    this.level.update(this.CANVAS_WIDTH, this.CANVAS_HEIGHT, this.worldTimeScale);
    ...
    this.chartBgOffset += 2.0 * this.worldTimeScale;
    if (this.chartBgOffset > this.CHART_BG_WIDTH) {
      this.chartBgOffset -= this.CHART_BG_WIDTH;
    }

    // Update x-axis scroll position
    this.xAxisOffset += this.X_AXIS_SCROLL_STEP * this.worldTimeScale;
    if (this.xAxisOffset > this.X_AXIS_REPEAT_WIDTH) {
      this.xAxisOffset -= this.X_AXIS_REPEAT_WIDTH;
    }
  ...
  private handleInput(): void {
    // --- 1. GAMEPAD ABRUFEN ---
    // Wir holen explizit Index 0 für den linken Controller (Player 1)
    const gamepads = navigator.getGamepads ? navigator.getGamepads() : [];
    const gp = gamepads[0]; // Dein Screenshot sagt: "Index Gamepad Links: 0"

    let moveLeft = false;
    let moveRight = false;
    let doJump = false;

    // --- 2. GAMEPAD LOGIK (Wenn angeschlossen) ---
    if (gp) {
      // A) STEUERUNG (AXIS 0)
      // Dein Screenshot: "Axis 0 = links/rechts (-1/1)"
      const xValue = gp.axes[ARCADE_MAP.AXIS_X];

      if (xValue < -0.5) moveLeft = true; // Stick nach Links
      if (xValue > 0.5) moveRight = true; // Stick nach Rechts

      // B) SPRINGEN DURCH STICK (AXIS 1)
      // Dein Screenshot: "Axis 1 = hoch/runter (-1/1)" -> Hoch ist meist negativ (-1)
      if (gp.axes[ARCADE_MAP.AXIS_Y] < -0.5) {
        doJump = true;
      }

      // C) SPRINGEN DURCH BUTTONS
      // Wir erlauben Y zum Springen – das ist Arcade-Standard
      if (gp.buttons[ARCADE_MAP.BTN_Y] && gp.buttons[ARCADE_MAP.BTN_Y].pressed) {
        doJump = true;
      }

      // D) START / PAUSE (Button 9)
      // Hier müssen wir aufpassen: Da handleInput 60x pro Sekunde läuft,
      // würde das Spiel sofort pausieren und wieder ent-pausieren.
      // Lösung: Einfache "Cooldown"-Logik oder nur im Menü nutzen (siehe unten).
      if (gp.buttons[ARCADE_MAP.BTN_START].pressed) {
        this.handleStartButtonPress();
      }
    }

    // --- 3. KEYBOARD FALLBACK (Bleibt erhalten) ---
    const keyLeft = this.keys['ArrowLeft'] || this.keys['a'];
    const keyRight = this.keys['ArrowRight'] || this.keys['d'];
    const keyJump =
      this.keys[' '] || this.keys['ArrowUp'] || this.keys['w'] || this.keys['Control'];
  ...
  private render(): void {
    ...
    // Draw chart background (scrolling pattern)
    if (this.chartBgImage && this.chartBgImage.complete && this.chartBgImage.naturalHeight !== 0) {
      const repeatCount = Math.ceil(this.CANVAS_WIDTH / this.CHART_BG_WIDTH) + 1;
      for (let i = -1; i < repeatCount; i++) {
        const xPos = i * this.CHART_BG_WIDTH - this.chartBgOffset;
        this.ctx.drawImage(this.chartBgImage, xPos, 0, this.CHART_BG_WIDTH, this.CANVAS_HEIGHT);
      }
    } else {
  ...
  onTwoPlayer() {
    console.log('2-Player not implemented.');
  }
  ...
  private pickRandomItemType(): SpecialItemType {
    const weighted: SpecialItemType[] = [
      'scoreBonus',
      'scoreBonus',
      'boost',
      'boost',
  ...
  // Start Button Methode
  private handleStartButtonPress(): void {
    const now = Date.now();
    // Verhindert "Dauerfeuer" (nur alle 500ms reagieren)
    if (now - this.lastStartPressTime < 500) return;

    this.lastStartPressTime = now;

    // Logik je nach Spielzustand
    if (this.gameState === 'menu' || this.gameState === 'gameover' || this.gameState === 'help') {
      // Spiel starten (simuliert klick auf "Start Game")
      // Falls du im Menü bist, starte Single Player
      if (this.gameState === 'menu') {
        this.startSingleFromMenu();
      } else if (this.gameState === 'help') {
        this.onHelpConfirmed();
      } else {
        this.startGame(); // Aus Gameover neustarten
      }
