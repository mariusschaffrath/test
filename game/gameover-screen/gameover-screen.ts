...
export class GameoverScreen implements OnInit, OnDestroy, AfterViewInit {
  @ViewChildren('menuBtn') menuBtns!: QueryList<ElementRef<HTMLButtonElement>>;
  selectedIndex: number = 0;

  ngAfterViewInit() {
    // Focus the first menu button if score is saved
    if (this.scoreSaved && this.menuBtns && this.menuBtns.length > 0) {
      setTimeout(() => {
  ...
  focusSelectedButton() {
    ...
    if (btns[this.selectedIndex]) {
      btns[this.selectedIndex].nativeElement.focus();
    }
  }

  keydownHandler(event: KeyboardEvent) {
    if (!this.scoreSaved) return;
    const btnCount = this.menuBtns?.length ?? 0;
    if (btnCount < 1) return;
    if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') {
      this.selectedIndex = (this.selectedIndex - 1 + btnCount) % btnCount;
      this.focusSelectedButton();
      event.preventDefault();
    } else if (event.key === 'ArrowRight' || event.key === 'ArrowDown') {
      this.selectedIndex = (this.selectedIndex + 1) % btnCount;
      this.focusSelectedButton();
      event.preventDefault();
    } else if (event.key === 'Enter' || event.key === ' ') {
      const btns = this.menuBtns!.toArray();
      btns[this.selectedIndex].nativeElement.click();
      event.preventDefault();
    }
  }
  ...
  saveScore() {
    ...
    this.highscoreService.addHighscore(this.playerName.trim(), this.score).subscribe({
      next: (savedEntry) => {
        ...
        this.loadHighscores();
        // Focus the first menu button after saving
        setTimeout(() => {
